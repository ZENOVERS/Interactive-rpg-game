...
int power_g = 5;
int money_g;
int God_stage_g = 0; //신룡을 클리어 하고나서부터 갈 수 있는 장소
...
void Monster_Way()
{
	...
	int monster_hp = (power_g + 3) - (rand() % 7); //몬스터를 이길 기준을, 캐릭터의 힘 플마3을 기준으로 랜덤 생성
...
void Unique_Event()
{
	...
	if (power_g >= 100)
	{
		...
		S(T);
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10);
		puts("네 약점은 이미 간파했다!! 받아라!!!!!!\n");
		S(T);
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 4);
		puts("아니 이럴수가..?!\n");
		S(T);
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10);
		puts("고대 신룡을 처지했다. 고대신룡의 가호를 손에 넣었다.\n");
		S(T);
		puts("공격력이 무려 100이 증가했다!\n");
		S(T);
		puts("새로운 스테이지, <신의 거처> 에도 갈 수 있게 됐다!!!\n");

		God_stage_g++;
	}
	else
	{
		puts("저 용을 이긴다면 이 곳에 들어갈 수 있겠지만, 너무 강해보인다. 적어도 공격력이 100은 넘어야 할 것 같다. 나중에 실력을 더 키워서 오자.\n");
		S(T + 500);
...
void God()
{
	...
	MandA
	puts("""여기는..?"" 지금까지 다녔던 장소하고는 사뭇 다른 느낌이 느껴진다.\n");
	S(T *2);
	puts("...! 내가 여기 당도할 것을 예측이라도 한 듯 신이 나를 기다리고 있다.\n");
	S(T * 2);
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 13);
	puts("[ ? : .... ] 신이 나를 응시한다.\n");
	S(T * 2);
	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 6);
	puts("..?! 눈빛의 중압감으로 숨과 몸을 컨트롤 할 수 없다.\n");
	S(T * 2);
	puts("신을 질린 듯 나를 보며 유유히 떠나갔다. 신이 상대해 주길 바란다면 적어도 공격력이 1000은 넘어야 할 것 같다.\n");
	S((T * 2) + 3000);
	cls;
}
